    ...
    <section ref={ref} id="brands" className="w-full bg-white overflow-hidden">
      ...
      >
        ...
        <div className="w-full bg-slate-50 py-4 overflow-hidden">
          <div className="flex gap-4 md:gap-8 lg:gap-12 brands-carousel">
            {duplicateBrands.map((brand, i) => (
              <motion.div
                key={`${brand.id}-${i}`}
                className="flex-shrink-0 w-56 md:w-64 lg:w-72 h-40 md:h-48 lg:h-56 flex items-center justify-center"
                whileHover={{
                  scale: 1.08,
              ...
              >
                <div className="w-full h-full flex items-center justify-center">
                  <img
                    src={brand.image}
                    alt={brand.id}
                    className="w-full h-full max-w-[92%] max-h-[92%] object-contain hover:scale-110 transition-transform duration-300 filter drop-shadow-sm hover:drop-shadow-md"
                  />
                </div>
              </motion.div>
            ))}
