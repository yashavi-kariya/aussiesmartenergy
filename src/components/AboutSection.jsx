...
//   return (
//     <section ref={ref} id="about" className="py-16 bg-white overflow-hidden">
...
//           {/* ── Right: GIF with Enhanced 3D Badge Overlays ── */}
//           <motion.div
...
//             <div className="relative w-full h-[400px] sm:h-[480px] rounded-3xl overflow-visible shadow-2xl group">
...
//               {/* Floating Animated Badges — Outside/Around Image */}
//               <div className="absolute inset-0 pointer-events-none" style={{ margin: '-60px' }}>
//                 {/* ── Badge 1: Reduce Bills — top-left ── */}
//                 <BadgeCard
//                   icon={<Zap className="w-5 h-5 text-[#39b54a]" />}
...
//                 <BadgeCard
//                   icon={<Leaf className="w-5 h-5 text-[#39b54a]" />}
//                   iconBg="bg-emerald-50"
//                   title="CO₂ Reduction"
//                   sub="Every kW Counts"
...
// /* Enhanced badge component with 3D animations and effects */
...
//       scale: 1.15,
//       y: -10,
//       boxShadow: "0 25px 50px rgba(57, 181, 74, 0.4)",
//       translateZ: 20
//     }}
//     transition={{
//       delay,
...
//       damping: 12
//     }}
//     className={`absolute ${className} bg-white/95 backdrop-blur-xl rounded-2xl px-4 py-3.5 shadow-2xl border border-white/40 flex items-center space-x-3 z-30 min-w-[170px] pointer-events-auto hover:shadow-3xl transition-all duration-300 group`}
//   >
...
//         delay: delay + 0.2
//       }}
//       className="absolute inset-0 bg-gradient-to-r from-emerald-300/30 via-blue-300/20 to-emerald-300/30 rounded-2xl -z-10 blur-md group-hover:blur-lg"
//     />
...
//         stiffness: 120
//       }}
//       className={`w-11 h-11 ${iconBg} rounded-xl flex items-center justify-center flex-shrink-0 group-hover:shadow-xl shadow-lg transition-all duration-300`}
//     >
//       {icon}
//     </motion.div>
...
//     >
//       <div className="text-[12px] font-extrabold text-[#1e2d53] leading-tight group-hover:text-[#39b54a] transition-colors duration-300">
//         {title}
//       </div>
...
const AboutSection = () => {
  const ref = useRef(null);
  // once: false → the whole text block replays its entrance animation
  // every time you scroll it back into view, not just the first time
  const isInView = useInView(ref, { once: false, amount: 0.3 });

  // Scroll-linked parallax: the text column drifts gently as you scroll
  // through the section, on top of (not instead of) the enter animation
  ...
  // Headline words "power on" one at a time, like a circuit switching on,
  // instead of the whole line fading in as a block
  ...
  // Each badge owns one accent color used consistently for icon, hover text, and glow.
  // Offsets are large enough that most of each badge sits OUTSIDE the image
  // (only a small sliver overlaps the corner) so nothing covers the diagram labels.
  const badgeData = [
    {
      icon: Zap,
      title: 'Reduce Bills',
